...
<body>
    ...
    if (isset($_SESSION["ID"]) && isset($_SESSION["role"]) && isset($_SESSION["occupation"])){
        if($_SESSION["role"]==="LEAVE-ADMIN"){
            ...
            $query=$con->prepare("SELECT `id`,`userId`,`Days`,`Reason`,`department`,`timeOfSubmission`,`status` FROM workleave WHERE status = -1");
            //AND status = -1
            $query->execute();
            ...
            $query2=$con->prepare("SELECT `id`,`userId`,`mcFile`,`Days`,`department`,`timeOfSubmission`,`status` FROM medicalcertificate WHERE status = -1");
            $query2->execute();
            ...
            while ($row = $result2->fetch_assoc()) {
                ...
                $status2 = $row['status'];
                echo "<th>$id2</th><th>$userId2</th><th>$mcFile2</th><th>$Days2</th><th>$department2</th><th>$timeOfSubmission2</th><th>$status2</th><th><a href='authoriseLeave.php?mcApproval=true&AMCID=".$id2."'>Approve</a></th><th><a href='authoriseLeave.php?mcDeny=true&DMCID=".$id2."'>Deny</a></th></tr>";
            }
            echo "</table></div>";
        ...
        elseif($_SESSION['occupation'] === "MANAGER"){
            ...
            // loading of Leave requests
            $query=$con->prepare("SELECT `id`,`userId`,`Days`,`Reason`,`department`,`timeOfSubmission`,`status` FROM workleave WHERE department = ? AND userId != ? AND status = -1");
            //AND status = -1
            ...
            echo "</table></div>";
            // loading of MC requests
            $query2=$con->prepare("SELECT `id`,`userId`,`mcFile`,`Days`,`department`,`timeOfSubmission`,`status` FROM medicalcertificate WHERE department = ? AND userId != ? AND status = -1");
